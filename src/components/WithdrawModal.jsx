...
const textFieldStyles = {
    '& .MuiOutlinedInput-root': {
        ...
        '& textarea, & input': { color: 'white', fontFamily: 'Kanit' },
...
const WithdrawModal = ({ open, onClose, item, userDisplayName, category }) => {
    ...
    const handleConfirm = async () => {
        if (!reason.trim() || quantity < 1 || isProcessing) return;

        setIsProcessing(true);

        const withdrawQuantity = Number(quantity);
        const currentQuantity = Number(item.quantity);
        const newQuantity = currentQuantity - withdrawQuantity;

        if (newQuantity < 0) {
            alert("จำนวนที่เบิกต้องไม่มากกว่าจำนวนคงเหลือ");
            setIsProcessing(false);
            return;
        }
        ...
        try {
            ...
            batch.set(transactionRef, {
                ...
                itemName: item.name,
                category: category,
                quantity: withdrawQuantity, // CORRECTED FIELD NAME
                reason: reason,
                user: userDisplayName, // CORRECTED FIELD NAME
                type: 'เบิก', // CORRECTED VALUE
            ...
            alert("เกิดข้อผิดพลาดในการดำเนินการเบิก");
            setIsProcessing(false);
        }
    };
        ...
        <Modal open={open} onClose={onClose}>
            <Box sx={modalStyle}>
                ...
                <Box component="form" noValidate autoComplete="off" sx={{ mt: 3 }}>
                    <Typography sx={{ fontFamily: 'Kanit', mb: 1, fontSize: '1rem' }}>จำนวนที่ต้องการเบิก</Typography>
                    <TextField
                        fullWidth
                        type="number"
                        value={quantity}
                    ...
                    <Typography sx={{ fontFamily: 'Kanit', mt: 2.5, mb: 1, fontSize: '1rem' }}>เหตุผล *</Typography>
                    <TextField
                        fullWidth
                        multiline
                        rows={4}
                        value={reason}
                        placeholder="กรุณาระบุเหตุผลในการเบิก"
                ...
                <Box sx={{ mt: 4, display: 'flex', justifyContent: 'flex-end', gap: 1.5 }}>
                    ...
                    <Button 
                        ...
                        disabled={!reason.trim() || quantity < 1 || isProcessing}
                        sx={{ 
