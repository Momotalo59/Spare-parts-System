...
const EditModal = ({ open, onClose, item, userDisplayName, originalCategory }) => {
    const [formData, setFormData] = useState({ name: '', category: '', quantity: '', imageUrl: '' });
    ...
    const handleConfirm = async () => {
        if (isProcessing || !formData.name) return;
        ...
        try {
            ...
            if (originalCategory !== newCategory) {
                // Category has changed: delete old doc, create new one
                const oldDocRef = doc(db, originalCategory, item.id);
                batch.delete(oldDocRef);

                const newDocRef = doc(db, newCategory, item.id); // Keep the same ID
                batch.set(newDocRef, updatedData);
            } else {
                // Category is the same: just update the existing doc
                const docRef = doc(db, originalCategory, item.id);
            ...
            batch.set(transactionRef, {
                ...
                quantity_change: 0, // No change in quantity from edit itself
                new_quantity: updatedData.quantity,
                reason: `แก้ไขข้อมูล (เดิม: ${originalCategory})`,
    ...
    return (
        <Modal open={open} onClose={onClose}>
            <Box sx={modalStyle}>
                <Typography sx={{ fontFamily: 'Kanit', fontWeight: 'bold', fontSize: '1.5rem', mb: 3 }}>แก้ไขรายการ</Typography>
                
                <TextField label="ชื่อรายการ" name="name" value={formData.name} onChange={handleChange} fullWidth sx={textFieldStyles} />
